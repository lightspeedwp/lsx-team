...
class LSX_TEAM_SCPO_Engine
{
    ...
    function __construct()
    {
        ...
        add_action('pre_get_posts', [$this, 'lsx_team_scporder_pre_get_posts']);

        add_filter('get_previous_post_where', [$this, 'lsx_team_scporder_previous_post_where']);
        add_filter('get_previous_post_sort', [$this, 'lsx_team_scporder_previous_post_sort']);
        add_filter('get_next_post_where', [$this, 'lsx_team_scporder_next_post_where']);
        add_filter('get_next_post_sort', [$this, 'lsx_team_scporder_next_post_sort']);

        add_filter('get_terms_orderby', [$this, 'lsx_team_scporder_get_terms_orderby'], 10, 3);
        add_filter('wp_get_object_terms', [$this, 'lsx_team_scporder_get_object_terms'], 10, 3);
        add_filter('get_terms', [$this, 'lsx_team_scporder_get_object_terms'], 10, 3);

    }//end __construct()


    function lsx_team_scporder_install()
    {
        global $wpdb;
    ...
    function _check_load_script_css()
    {
        ...
        if (!empty($objects)) {
            if (isset($_GET['post_type']) && !isset($_GET['taxonomy']) && array_key_exists(sanitize_text_field(wp_unslash($_GET['post_type'])), $objects)) {
                // if page or custom post types
                $active = true;
            }

            if (!isset($_GET['post_type']) && strstr(sanitize_text_field(wp_unslash($_SERVER['REQUEST_URI'])), 'wp-admin/edit.php') && array_key_exists('post', $objects)) {
                // if post
                $active = true;
            }
        }
    ...
    }//end _check_load_script_css()


    function load_script_css()
    {
        if ($this->_check_load_script_css()) {
            wp_enqueue_script('scporderjs', LSX_TEAM_URL.'assets/js/scporder.min.js', ['jquery', 'jquery-ui-sortable'], null, true);
            ...
            wp_enqueue_style('scporder', LSX_TEAM_URL.'assets/css/scporder.css', [], null);
        }
    ...
    function refresh()
    {
        ...
        if (!empty($tags)) {
            foreach ($tags as $taxonomy => $taxonomy_data) {
                ...
                foreach ($results as $key => $result) {
                    $wpdb->update($wpdb->terms, ['lsx_team_term_order' => ($key + 1)], ['term_id' => $result->term_id]);
                }
            }//end foreach
        }//end if
    ...
    function update_menu_order_tags()
    {
        ...
        foreach ($data as $key => $values) {
            foreach ($values as $position => $id) {
                $wpdb->update($wpdb->terms, ['lsx_team_term_order' => $menu_order_arr[$position]], ['term_id' => intval($id)]);
            }
        }

    }//end update_menu_order_tags()


    function lsx_team_scporder_previous_post_where($where)
    {
        global $post;
        $objects = $this->get_lsx_team_scporder_options_objects();
    ...
    }//end lsx_team_scporder_previous_post_where()


    function lsx_team_scporder_previous_post_sort($orderby)
    {
        global $post;
        $objects = $this->get_lsx_team_scporder_options_objects();
    ...
    }//end lsx_team_scporder_previous_post_sort()


    function lsx_team_scporder_next_post_where($where)
    {
        global $post;
        $objects = $this->get_lsx_team_scporder_options_objects();
    ...
    }//end lsx_team_scporder_next_post_where()


    function lsx_team_scporder_next_post_sort($orderby)
    {
        global $post;
        $objects = $this->get_lsx_team_scporder_options_objects();
    ...
    }//end lsx_team_scporder_next_post_sort()


    function lsx_team_scporder_pre_get_posts($wp_query)
    {
        $objects = $this->get_lsx_team_scporder_options_objects();
    ...
    }//end lsx_team_scporder_pre_get_posts()


    function lsx_team_scporder_get_terms_orderby($orderby, $args)
    {
        if (is_admin()) {
            return $orderby;
        }
    ...
    }//end lsx_team_scporder_get_terms_orderby()


    function lsx_team_scporder_get_object_terms($terms)
    {
        $tags = $this->get_lsx_team_scporder_options_tags();
    ...
    }//end lsx_team_scporder_get_object_terms()
    ...
    function get_lsx_team_scporder_options_objects()
    {
        return [
            'team' => esc_html_x('Team Member', 'post type singular name', 'lsx-team'),
        ];

    }//end get_lsx_team_scporder_options_objects()


    function get_lsx_team_scporder_options_tags()
    {
        return [
            'team_role' => esc_html_x('Role', 'taxonomy singular name', 'lsx-team'),
        ];

    }//end get_lsx_team_scporder_options_tags()
...
}//end lsx_team_scporder_uninstall()
...
}//end lsx_team_scporder_uninstall_db()
