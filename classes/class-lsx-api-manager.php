...
class LSX_API_Manager
{
    ...
    public function __construct($api_array=[])
    {
        ...
        add_filter('plugin_action_links_'.plugin_basename(str_replace('.php', '', $this->file).'/'.$this->file), [$this, 'add_action_links']);
        $this->status = get_option($this->product_slug.'_status', false);

        if (isset($_GET['page']) && in_array($_GET['page'], apply_filters('lsx_api_manager_options_pages', [false]))) {
        ...
        add_action('in_plugin_update_message-'.$this->file, [$this, 'plugin_update_message'], 10, 2);

        if (function_exists('tour_operator')) {
            add_action('lsx_to_framework_api_tab_content', [ $this, 'dashboard_tabs' ], 1, 1);
        } else {
            add_action('lsx_framework_api_tab_content', [ $this, 'dashboard_tabs' ], 1, 1);
        }

        add_action('wp_ajax_wc_api_'.$this->product_slug, [$this, 'activate_deactivate']);
        add_action('wp_ajax_nopriv_wc_api_'.$this->product_slug, [$this, 'activate_deactivate']);
...
?>
<p><small class="messages" style="font-weight:normal;"><?php echo implode('. ', $this->messages); ?></small></p>
    <?php
                }
?>
    ...
    public function check_status($response=false)
    {
        ...
        if (is_object($response)) {
            ...
            } else if (isset($response->status_check)) {
